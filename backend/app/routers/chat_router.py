...
# Keep the legacy endpoint for backward compatibility but mark it as deprecated
@router.post("/{user_id}", response_model=ChatResponse, summary="[DEPRECATED] Send a message to the AI agent")
async def send_message_deprecated(
    ...
    print(f"=== CHAT POST ENDPOINT CALLED (DEPRECATED) ===")
...
@router.get("/history/{user_id}", response_model=List[ChatMessageRead], summary="[DEPRECATED] Retrieve chat history for a user")
async def get_chat_history_deprecated(
    user_id: UUID,
    current_user: User = Depends(get_current_active_user),
    session = Depends(get_session)
):
    print(f"=== CHAT HISTORY GET ENDPOINT CALLED (DEPRECATED) ===")
    ...
    if current_user.user_id != user_id:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to view chat history for this user."
        )
