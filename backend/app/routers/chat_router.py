...
async def get_formatted_chat_history(chat_id: UUID, session: Session, limit: int = 10):
    db_messages = await get_chat_history_from_db(chat_id, session, limit=limit)
    formatted_history = []
    for msg in db_messages:
        # Simple format without langchain dependencies
...
@router.post("/{user_id}", response_model=ChatResponse, summary="Send a message to the AI agent")
...
@router.get("/history/{user_id}", response_model=List[ChatMessageRead], summary="Retrieve chat history for a user")
